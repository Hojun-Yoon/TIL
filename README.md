...
2019년 12월 30일
----------------
ㅇ 리액트는 서버 렌더링 제공, 클라이언트에서 준비한 컴포넌트 코드를 node.js 에서 사용해야 하며, jsx 와 es6 문법을
사용해야 한다.

ㅇ node v8 버전 이후 사용이 어려워, webpack 을 활용하여 바벨로더를 적용한 코드를 변환하고 하나의 번들 파일을 
만들게 되면 리액트 코드를 서버에서 쉽게 불러와서 사용할 수 있음.

ㅇ 단, 서버에서도 render 함수를 만들어서 컴포넌트를 렌더링 해준다.

ㅇ node.js 와 mongodb 간 궁합이 잘 맞는다고 함.

2019년 12월 31일
----------------
ㅇ history 모듈이 설치되면, static router, rendertoString 등과 충돌이 발생.

ㅇ 미들웨어는 액션이 dispatch 되어서, 리듀서에서 처리하기 전에 사전에 지정된 작업들을 의미한다.
- 액션과 리듀서 사이의 중간자
- 전달받은 액션을 콘솔에 기록할 수 있고, 전달받은 액션에 기반해 액션을 취소할 수 있고, 다른 종류의 액션들을
추가 디스패치할 수 있다. 
...
2020년 1월 12일
-------------------
ㅇ %d 는 숫자, %c 는 문자, 입력받는 숫자 및 문자에는 & 엔드퍼센트를 사용한다. 

ㅇ %02d를 사용하면 2칸을 사용해 출력하는데, 한 자리 수인 경우 앞에 0을 붙여 출력한다.

2020년 1월 15일
----------------
ㅇ indexOf 로 똑같은 형태의 배열과 객체를 조회해도 -1(없음)으로 표시된다.

ㅇ 배열, 객체의 주소값과 상관없이 비교하기 위해서 JSON.stringify 를 사용하였음.

ㅇ Object.keys()는 개체 고유 속성의 이름을 배열로 반환한다.

ㅇ 재귀에서는 초입점, 종료지점, 반복되는 구문 세가지를 항상 고민하면서 사용하자.

ㅇ 알고리즘, 정말 중요한 경우의 수(핵심)을 잡으려고 노력하자. 버릴 수 있는 큰 덩어리를 제거해나가기.

2020년 2월 7일
-------------------------

ㅇ 비트단위(bitwise) 연산자는,
~(bitwise not)- tilde 틸드라고 읽는다., &(bitwise and), |(bitwise or), ^(bitwise xor),
<<(bitwise left shift), >>(bitwise right shift)
- xor 연산자는 두 비트가 다를 때 참으로 표현되는 것.

2020년 2월 8일
---------------
ㅇ 비트단위 and 연산은 네트워크에 연결되어 있는 두 개의 컴퓨터가 데이터를 주고받기 위해
같은 네트워크에 있는 지 아닌지를 판단

ㅇ 빠른 계산이 필요한 그래픽처리에서 마스크연산(특정 부분을 가리고 출력하는)
을 수행하는 데에도 효과적으로 사용.

ㅇ xor 연산자(배타적 논리합) 두 장의 이미지가 겹쳤을 때 서로 다른 부분 체크
- 효과적인 그림 표현, 비행기 슈팅게임

ㅇ 짧은 코드로 문제를 해결할 때 삼항 연산자는 유용하게 사용됨.

2020년 2월 11일
--------------
ㅇ switch( ) 에 주어지는 값은 “정수"값만 가능하며,
문자도 아스키코드 정수값이기 때문에 가능하다.

ㅇ swtich( ).. case ... break; 제어문에서
break;를 제거하면 멈추지 않고 다음 명령이 실행되는 특성을 이용할 수 있다.

2020년 2월 12일
---------------
ㅇ goto 명령은 반복 실행을 만들어낼 수 있는 가장 간단한 명령이지만,
복잡하게(스파게티 코드) 사용하는 경우, 이해가 어렵고 오류가 생기기 쉽다.

2020년 2월 18일
----------------
ㅇ 암달의 법칙 : 멀티코어를 사용하는 프로그램의 속도는 프로그램 내부에 존재하는 순차적
부분이 사용하는 시간에 의해서 제한된다.

- 여러 개의 스레드가 동시에 작업을 수행하더라도
Synchronized 블록이나 데이터베이스, 네트워크 API 호출 등을 만날 때
다른 스레드와 나란히 줄을 서서 순차적으로 작업을 수행해야 하는
경우도 있음.  

- 암달의 법칙은 속도의 상한이 이런 순차적 코드가 사용하는 시간에 의해서
제한된다.

- 순차적 코드를 블로킹(blocking) 콜이라고 한다. 

2020년 2월 29일
---------------
ㅇ 한글 인코딩이 안되는 문제(파이썬)
- #-*- coding: utf-8 -*-

2020년 3월 1일
---------------
ㅇ Shell 이란 : 명령어 처리기
운영 체제 상에서 다양한 운영 체제 기능과 서비스를 구현하는 인터페이스를
제공하는 프로그램
(사람이 컴퓨터에게 어떤 일을 시킬 때 쓰는 프로그램)

ㅇ Shell Script
- 텍스트 형식으로 저장되는 프로그램, 한 줄씩 순차적으로 읽어 실행되도록 작성된 프로그램
- 스크립트 : 인터프리터 방식으로 동작하는 컴파일 되지 않는 프로그램

2020년 5월 24일
----------------
ㅇ 오버로딩 : 같은 이름의 메소드를 여러 개 가지면서 매개변수의 
유형과 개수가 다르도록 하는 기술(메소드 오버로딩, 생성자 오버로딩)

ㅇ 오버라이딩 : 상위 클래스가 가지고 있는 메소드를 하위 클래스가
재정의 해서 사용
